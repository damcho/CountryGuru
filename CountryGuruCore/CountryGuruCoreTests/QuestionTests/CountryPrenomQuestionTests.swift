...
struct CountryPrenomQuestionTests: InquirySpecs {
    ...
    func query_path() async throws {
        #expect(
            anyCountryPrenomQUestion.makeURL(from: anyURL)
                .absoluteString == "\(anyURL.absoluteString)/all?fields=name"
        )
    }
    ...
    func maps_response_successfully() async throws {
        ...
        #expect(try anyCountryPrenomQUestion.mappedResponse(
            from: countryNameData,
            httpURLResponse: validHTTPURLResponse
        ) == .multiple(["Argentina"]))
    }

    @Test
    func returns_emtpy_response_message_on_empty_data() async throws {
        let countryNameData = #"[]"#.data(using: .utf8)!
        #expect(try anyCountryPrenomQUestion.mappedResponse(
            from: countryNameData,
            httpURLResponse: validHTTPURLResponse
        ) == .multiple(["No countries match with your query"]))
    }
}
