//
//  CountryCapitalQuestionTests.swift
...
struct CountryCapitalQuestionTests: InquirySpecs {
    ...
    func maps_response_successfully() async throws {
        ...
        #expect(
            try sut
                .mappedResponse(from: countryCapital.http, httpURLResponse: validHTTPURLResponse) == countryCapital
                .domain
        )
    }

    @Test
    func returns_default_response_on_empty_data() async throws {
        let aCountry = "Argentina"
        let sut = CountryCapitalQuestion(countryName: aCountry)
